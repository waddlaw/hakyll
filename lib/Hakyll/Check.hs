...
import Control.Exception
       (SomeAsyncException(..), SomeException(..), throw, try)
...
instance Monoid CheckerWrite where
  ...
  mappend (CheckerWrite f1 o1) (CheckerWrite f2 o2) = CheckerWrite (f1 + f2) (o1 + o2)
...
--------------------------------------------------------------------------------
runChecker :: Checker a -> Configuration -> Logger -> Check -> IO (a, CheckerState)
runChecker checker config logger check' = do
  let read' = CheckerRead {checkerConfig = config, checkerLogger = logger, checkerCheck = check'}
  Logger.flush logger
...
checkDestination = do
  ...
  files <- liftIO $ getRecursiveContents (const $ return False) (destinationDirectory config)
  let htmls = [destinationDirectory config </> file | file <- files, takeExtension file == ".html"]
...
requestExternalUrl url =
  ...
  where
    ...
    ua =
      fromString $
      "hakyll-check/" ++ (intercalate "." $ map show $ versionBranch Paths_hakyll.version)
#else
