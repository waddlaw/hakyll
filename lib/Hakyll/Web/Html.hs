...
import System.FilePath.Posix (joinPath, splitPath, takeDirectory)
...
-- | Map over all tags in the document for Monad
withTagsM
  :: Monad m
  => (TS.Tag String -> m (TS.Tag String)) -> String -> m String
withTagsM f = fmap renderTags' . mapM f . parseTags'
...
renderTags' =
  ...
  where
    minimize =
      S.fromList ["area", "br", "col", "embed", "hr", "img", "input", "meta", "link", "param"]
...
toSiteRoot = emptyException . joinPath . map parent . filter relevant . splitPath . takeDirectory
  where
    parent = const ".."
    emptyException [] = "."
    emptyException x = x
